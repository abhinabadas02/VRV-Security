...
//    @Autowired
//    private BloodFormRepo bloodFormRepo;

    @Autowired
    public UserManagamentService(UserRepo usersRepo) {
    ...
    public ReqRes login(ReqRes loginRequest){
        ReqRes response = new ReqRes();
        OurUsers user1=usersRepo.findById(loginRequest.getId()).orElseThrow();
        ...
        try {
            if (isRight){
                Optional<OurUsers> ourUsers=usersRepo.findOneByIdAndPassword(loginRequest.getId(),encodedpassword);
                if (ourUsers.isPresent()){
                    var user = usersRepo.findById(loginRequest.getId()).orElseThrow();
            ...
            else throw new Exception("Username or Password Not Matched");
